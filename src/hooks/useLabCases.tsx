...
export function useLabCaseStats() {
  ...
  const stats = {
    ...
    paidFees: cases.filter((c) => c.is_paid).reduce((sum, c) => sum + Number(c.net_amount || c.lab_fee), 0),
  };
...
export function useCreateLabCase() {
  ...
  return useMutation({
    mutationFn: async (labCase: any) => {
      ...
      const { data, error } = await (supabase as any)
        .from("lab_cases")
        .insert({ ...labCase, registered_by: user?.id, registered_by_name: registeredByName })
        .select()
        .single();
      if (error) throw error;
      return data;
